...
public class IncludeGuardDetection
{
    ...
    private static char[] findNotDefined(Lexer l)
    ...
    private static boolean checkToken(Token t, char[] image)
            throws OffsetLimitReachedException
    {
    ...
    private static Token skipAll(Lexer l, int kind)
            throws OffsetLimitReachedException
    {
        // Skip empty lines
        Token t = l.nextToken();
    ...
    private static boolean findIncludeEndif(Lexer l)
            throws OffsetLimitReachedException
    {
        if (skipAll(l, Lexer.tNEWLINE).getType() != IToken.tPOUND) {
            return false;
