...
public class PDOMTagIndex
{
    ...
    public PDOMTagIndex(Database db, long ptr)
    ...
    private long getFieldAddress(Fields field)
            throws CoreException
    {
        if (rootRecord == 0) {
    ...
    private PDOMStringSet getTaggerIds()
            throws CoreException
    {
        if (taggerIds == null) {
    ...
    private Iterable<ITag> getTags(long binding_record)
    {
        ...
        return new BTreeIterable<ITag>(btree, new BTreeIterable.Descriptor<ITag>()
        {
            ...
            @Override
            public int compare(long test_record)
                    throws CoreException
            {
