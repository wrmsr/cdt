...
import org.eclipse.cdt.core.parser.scanner.MacroDefinitionParser;
...
public class PDOMMacro
        implements IIndexMacro, IPDOMBinding, IASTFileLocation
{
    ...
    public PDOMMacroContainer getContainer()
            throws CoreException
    {
        if (fContainer == null) {
    ...
    private IString getExpansionInDB()
            throws CoreException
    {
    ...
    private IString getParamListInDB()
            throws CoreException
    {
        Database db = fLinkage.getDB();
    ...
    public void setNextMacro(PDOMMacro macro)
            throws CoreException
    {
        setNextMacro(macro != null ? macro.getRecord() : 0);
    }

    private void setNextMacro(long rec)
            throws CoreException
    {
        fLinkage.getDB().putRecPtr(fRecord + NEXT_IN_FILE, rec);
    }

    private PDOMMacro getPrevInContainer()
            throws CoreException
    {
        return getMacroField(PREV_IN_CONTAINER);
    }

    void setPrevInContainer(PDOMMacro macro)
            throws CoreException
    {
        setMacroField(PREV_IN_CONTAINER, macro);
    }

    public PDOMMacro getNextInContainer()
            throws CoreException
    {
        return getMacroField(NEXT_IN_CONTAINER);
    }

    void setNextInContainer(PDOMMacro macro)
            throws CoreException
    {
        setMacroField(NEXT_IN_CONTAINER, macro);
    }

    private void setMacroField(int offset, PDOMMacro macro)
            throws CoreException
    {
    ...
    private PDOMMacro getMacroField(int offset)
            throws CoreException
    {
