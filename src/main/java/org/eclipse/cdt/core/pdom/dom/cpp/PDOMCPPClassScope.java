...
import org.eclipse.cdt.core.dom.parser.cpp.CPPClassScope;
...
import org.eclipse.cdt.core.index.DeclaredBindingsFilter;
...
class PDOMCPPClassScope
        implements ICPPClassScope, IIndexScope
{
    ...
    public static void updateCache(IPDOMCPPClassType ct, PDOMNode member)
            throws CoreException
    {
    ...
    public static CharArrayMap<List<PDOMBinding>> getBindingMap(IPDOMCPPClassType ct)
            throws CoreException
    {
        final Long key = ct.getRecord() + PDOMCPPLinkage.CACHE_MEMBERS;
        final PDOM pdom = ct.getPDOM();
