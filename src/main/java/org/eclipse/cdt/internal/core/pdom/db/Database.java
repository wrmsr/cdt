...
public class Database
{
     ...
     */
    public Database(File location, ChunkCache cache, int version, boolean openReadOnly)
            throws CoreException
    {
        try {
            fLocation = location;
    ...
    void read(ByteBuffer buf, long position)
            throws IOException
    {
        int retries = 0;
        do {
    ...
    void write(ByteBuffer buf, long position)
            throws IOException
    {
        int retries = 0;
        while (true) {
    ...
    private void reopen(ClosedChannelException e, int attempt)
            throws ClosedChannelException, FileNotFoundException
    {
    ...
    public void transferTo(FileChannel target)
            throws IOException
    {
        assert fLocked;
     ...
     * @throws CoreException
     */
    public Chunk getChunk(long offset)
            throws CoreException
    {
    ...
    private void databaseCorruptionDetected()
            throws CoreException
    {
     ...
     */
    public long malloc(final int datasize)
            throws CoreException
    {
        assert fExclusiveLock;
     ...
     */
    private long createNewChunks(int numChunks)
            throws CoreException
    {
        assert fExclusiveLock;
        synchronized (fCache) {
    ...
    private void removeBlock(Chunk chunk, int blocksize, long block)
            throws CoreException
    {
        assert fExclusiveLock;
    ...
    private void addBlock(Chunk chunk, int blocksize, long block)
            throws CoreException
    {
        assert fExclusiveLock;
        // Mark our size
    ...
    private void putFreeRecPtr(long offset, long value)
            throws CoreException
    {
        getChunk(offset).putFreeRecPtr(offset, value);
    }

    private long getFreeRecPtr(long offset)
            throws CoreException
    {
    ...
    public void clearBytes(long offset, int byteCount)
            throws CoreException
    {
        getChunk(offset).clear(offset, byteCount);
    }

    public void putBytes(long offset, byte[] data, int len)
            throws CoreException
    {
        getChunk(offset).put(offset, data, len);
    }

    public void putBytes(long offset, byte[] data, int dataPos, int len)
            throws CoreException
    {
        getChunk(offset).put(offset, data, dataPos, len);
    }

    public void getBytes(long offset, byte[] data)
            throws CoreException
    {
        getChunk(offset).get(offset, data);
    }

    public void getBytes(long offset, byte[] data, int dataPos, int len)
            throws CoreException
    {
        getChunk(offset).get(offset, data, dataPos, len);
    }

    public IString newString(String string)
            throws CoreException
    {
        return newString(string.toCharArray());
    }

    public IString newString(char[] chars)
            throws CoreException
    {
    ...
    public IString getString(long offset)
            throws CoreException
    {
    ...
    public void giveUpExclusiveLock(final boolean flush)
            throws CoreException
    {
        if (fExclusiveLock) {
            try {
    ...
    private void flushAndUnlockChunks(final ArrayList<Chunk> dirtyChunks, boolean isComplete)
            throws CoreException
    {
    ...
    private void markFileIncomplete()
            throws CoreException
    {
